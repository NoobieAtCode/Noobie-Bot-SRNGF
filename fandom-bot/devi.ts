...
const icomp = async (devdata: Array<idevdata>) => {
  ...
  for (let k of devdata) {
    let urlfile = await Deno.readFile(
      path.join(__rootdir, `devicomp/url-${k.name}.png`),
    );
    let dbfile = await Deno.readFile(
      path.join(__rootdir, `devicomp/db-${k.name}.png`),
    );
    ...
    switch (input) {
      ...
      case ".devi":
        await main().catch(async (err) => {
          if (
            /^(NotFound\: The system cannot find the file specified\.)/g.test(
              err,
            )
